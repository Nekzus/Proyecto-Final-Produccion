...
import { totalAmount, productContainer, slidersContainer, totalQuantity, cartContainer, historyContainer, personalInformation, modalCheckout } from '../modules/selectors.js';
//**IMPORTACION FUNCIONES */
import { borrarItem, montoTotalProductos, indicadorCantidad, mensajeCarroVacio, productCart, cambiarCantidad, marcarFecha, darFormatoMoneda, mensajeMisCompras } from '../modules/functions.js';
...
export const renderizarProductos = (data) => {
    ...
    data.forEach((item) => {
        ...
        const headerPlatform = document.querySelector(`span.cabecera-plataforma[id='plataforma-${item.id}']`);// Se selecciona el elemento cabecera de la plataforma. 
        ...
        const availableStock = document.querySelector(`p.stock-disponible[id='stock-${id}']`); // Se selecciona el elemento stock.
        if (stock <= 0) {
            ...
            availableStock.innerHTML = '<span class="badge stock-item badge-sin-stock rounded-pill">SIN STOCK</span>'; // Se cambia el color del badge.
...
export const renderizarHistorial = (data) => {
    ...
    data.forEach((item) => {
        ...
        auth.onAuthStateChanged(user => { // Se verifica si el usuario está logueado.
            if (user !== null) {
                if (user.email === email) { // Se verifica si el email del usuario es igual al email de la transacción.
                    ...
                    product.forEach((product) => { // Se recorre el array de productos.
...
export const renderizarDatosPersonales = (data) => { // Se renderiza los datos personales del usuario.
...
export const renderizarModalCheckout = (data) => { // Se renderiza el modal de checkout.
